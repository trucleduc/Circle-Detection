...
void myLsd(char *input_buf, char *input_buf2)
{
  /* LSD parameters */
  double scale = 0.8;       /* Scale the image by Gaussian filter to 'scale'. */
  double * out;
  ...
  Mat img=imread(input_buf,0);
  
  int X=img.cols;
  int Y=img.rows  ;
  /* create a simple image: left half black, right half gray */
  double *image = (double *) malloc( img.rows * img.cols * sizeof(double) );
  if( image == NULL )
  {
  ...
  for(int x=0;x<X;x++)
      for(int y=0;y<Y;y++)
          image[x+y*X] = img.at<uchar>(y,x); /* image(x,y) */
  ...
  Mat tmpMat=Mat::zeros(img.rows, img.cols, CV_8UC1);
  
  for(i=0;i<n;i++)
      
  {
      if(norm(Point2f(out[7*i],out[7*i+1])-Point2f(out[7*i+2],out[7*i+3]))<40) continue;
      line(tmpMat,Point2f(out[7*i],out[7*i+1]),Point2f(out[7*i+2],out[7*i+3]),Scalar(255,0,0));             
  }
  
  //imshow("test",tmpMat);
  imwrite(input_buf2,tmpMat);
   
}


/** Mex function for lsd
 * mex LSD.cpp -ID:\OpenCV2.4.5\build\include -LD:\OpenCV2.4.5\build\x64\vc11\lib -lopencv_core245.lib opencv_highgui245.lib
 * input: UINT8 image
 * output: number of detected line segments
 *         line paramenters: (x1,y1) (x2,y2)
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
   ...
   // fopen_s(&file,"log.txt","w");
    ...
    char *input_buf,*input_buf2, *output_buf;
    size_t buflen;
	...
	if(mxGetClassID(prhs[0])!=9)
    {
        mexErrMsgIdAndTxt( "MATLAB:revord:dataType",
              "UINT8 required");
    }
    ...
    uchar *dt=(uchar*)mxGetData(prhs[0]);
    ...
    double* out=lsd(&n, data,N,M);

    tmpMat=Mat::zeros(M,N, CV_8UC1);
    
    printf("%i\n",n);

    double* endPs=(double*)malloc(4*n*sizeof(double));
    for(i=0;i<n;i++)   
    {
        line(tmpMat,Point2f(out[7*i],out[7*i+1]),Point2f(out[7*i+2],out[7*i+3]),Scalar(255,0,0));
       // printf("%lf %lf %lf %lf\n",out[7*i],out[7*i+1],out[7*i+2],out[7*i+3]);
        
        endPs[4*i]=out[7*i];endPs[4*i+1]=out[7*i+1];endPs[4*i+2]=out[7*i+2];endPs[4*i+3]=out[7*i+3];
        // std::cout<<out[7*i]<<std::endl;
    }
    ...
    double* B2 = (double*)mxGetPr(plhs[0]);
    memcpy(B2, B, M*N*sizeof(double));
    
    plhs[1] = mxCreateDoubleMatrix(4, n, mxREAL);
    double *B3=(double*)mxGetPr(plhs[1]);
    for(int i=0;i<4*n;i++){
        B3[i]=endPs[i];
